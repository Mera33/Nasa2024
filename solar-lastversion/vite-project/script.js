...
function showPlanetInfo(planet) {
  ...
  // إنشاء زر لفتح الرابط
  var openLinkButton = document.createElement('button');
  openLinkButton.innerText = 'Open Planet Details';
  openLinkButton.style.marginTop = '10px';
  openLinkButton.onclick = function() {
    window.open(planetData[planet].url, '_blank');
  };

  // إضافة الزر إلى الـ info
  info.appendChild(openLinkButton);
  info.style.display = 'block';
}
...
function closeInfo() {
  ...
  // إزالة زر فتح الرابط من info
  var openLinkButton = info.querySelector('button');
  if (openLinkButton) {
    info.removeChild(openLinkButton);
  }
...
async function loadConstellationData() {
  try {
    ...
    // Loop through each constellation and create stars
    data.constellations.forEach(constellation => {
      // Create a group for each constellation to hold its stars and lines
      const constellationGroup = new THREE.Group();
      constellationGroup.name = constellation.name;

      // Create stars for the constellation
      constellation.stars.forEach(starData => {
        ...
        // Add star to the constellation group
        constellationGroup.add(starMesh);
      });

      // Create lines between stars in the constellation realistically
      if (constellation.connections) {
        constellation.connections.forEach(connection => {
          const startStar = constellation.stars.find(star => star.id === connection[0]);
          const endStar = constellation.stars.find(star => star.id === connection[1]);

          if (startStar && endStar) {
            const startPoint = new THREE.Vector3(startStar.x, startStar.y, startStar.z);
            const endPoint = new THREE.Vector3(endStar.x, endStar.y, endStar.z);

            const lineGeometry = new THREE.BufferGeometry().setFromPoints([startPoint, endPoint]);
            const lineMaterial = new THREE.LineBasicMaterial({ color: 0x00bfff, transparent: true, opacity: 0.6 });
      ...
      // Add the constellation group to the scene
...
// Call the function to load constellations
